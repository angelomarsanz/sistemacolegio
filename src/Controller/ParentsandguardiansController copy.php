...
class ParentsandguardiansController extends AppController
{
    ...
    {
        $representantes = $this->Parentsandguardians->find('all')->where(['Parentsandguardians.fiscal_address !=' => '', 'Parentsandguardians.tax_phone !=' => '']);

		$contadorRepresentantes = $representantes->count();

		$this->Flash->success(__('Total representantes encontrados ' . $contadorRepresentantes));

        if ($contadorRepresentantes > 0)
        {
            foreach ($representantes as $representante) 
            {
                $representante->fiscal_address = '';
                $representante->tax_phone = '';
                if (!($this->Parentsandguardians->save($representante))) 
                {
                    $this->Flash->error(__('Id Representante no guardado ' . $representante->id));
                }
            }
        }
    }
    ...
    public function findFamily()
    {
        if ($this->request->is('ajax')) {
            ...
            $results = $this->Parentsandguardians->find('all', [
                'conditions' => [['identidy_card LIKE' => $name . '%'], ['guardian !=' => 1]]]);
            $resultsArr = [];
            foreach ($results as $result) {
				$resultsArr[] =['label' => $result['identidy_card'] . ' (' . $result['surname'] . ' ' . $result['first_name'] . ')', 'value' => $result['identidy_card'] . ' (' . $result['surname'] . ' ' . $result['first_name'] . ')', 'id' => $result['id']];
				}
			exit(json_encode($resultsArr, JSON_FORCE_OBJECT));
        }
    }

    public function busquedaApellidoRepresentante()
    {
        if ($this->request->is('ajax')) {
            $this->autoRender = false;
            $name = trim($this->request->query['term']);
    ...
    public function busquedaApellidoAlumno()
    {
        if ($this->request->is('ajax')) {
            $this->autoRender = false;
            ...
            $results = $this->Students->find('all', [
                'conditions' => [['surname LIKE' => $name . '%'], ['id >' => 1]]]);
            $resultsArr = [];
            foreach ($results as $result) {
				$resultsArr[] =['label' => $result['surname'] . ' ' . $result['first_name'] . ' (' . $result['type_of_identification'] . $result['identity_card'] . ')', 'value' => $result['surname'] . ' ' . $result['first_name'] . ' (' . $result['type_of_identification'] . $result['identity_card'] . ')', 'id' => $result['parentsandguardian_id']];
				}
			exit(json_encode($resultsArr, JSON_FORCE_OBJECT));
        }
    }
