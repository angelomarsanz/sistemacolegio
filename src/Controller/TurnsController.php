...
class TurnsController extends AppController
{
		...
		else
		{
			...
			if ($codigoRetornoResultado == 0)
			{
				foreach ($pagosFacturas as $pago)
				{
					...
					elseif ($pago->payment_type == "Efectivo" && $pago->moneda == "P")
					{
    ...
    {     
        ...
        $pagosMes = $this->Payments->find('all')
			->contain(['Bills'])
		...
		{
			$fechaActual = $pago->bill->date_and_time->format('d/m/Y');
			...
			$documentosFiscales[$fechaActual][] = 
				[
					...
					'numero_factura'	=> $pago->bill->bill_number,
					'numero_control' 	=> $pago->bill->control_number,
					'fiscal' 			=> $pago->bill->fiscal,
					'tipo_documento' 	=> $pago->bill->tipo_documento,
					'anulado' 			=> $pago->bill->annulled,
					'moneda' 			=> $pago->moneda,
					'monto' 			=> $pago->amount
				];
			/*
