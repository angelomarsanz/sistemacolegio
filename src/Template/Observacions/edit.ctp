...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li><?= $this->Html->link(__('Lista de calificaciones descriptivas de la materia'), ['controller' => 'Observacions', 'action' => 'index']) ?></li>
    </ul>
</nav>
<div class="observacions form large-9 medium-8 columns content">
    <?= $this->Form->create($observacion) ?>
    <fieldset>
        <legend><?= __('Modificar calificación descriptiva de la materia') ?></legend>
        <?php
            echo $this->Form->input('lapso_id', ['options' => $lapsos, 'value' => $idLapso]);
            echo $this->Form->input('materia_id', ['options' => $materias, 'value' => $idMateria]);
