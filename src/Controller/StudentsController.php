...
class StudentsController extends AppController
{
    ...
    public function testFunction2()
    {
				...
				if ($contadorTransacciones > 0)
				{
					...
					if ($septiembreEncontrado == 0)
					{
						foreach ($transaccionesEstudiante as $transaccion)
						{
							...
							{
								$this->Flash->error(__('La transacción con el id ' . $transaccion->id . ' no pudo ser eliminada'));
							}				
							else
							{ 
								$contadorEliminadas++;
			...
			foreach ($estudiantes as $estudiante)
			{
				...
				if ($contadorTransacciones > 0)
				{
					...
					if ($septiembreEncontrado == 0)
					{
						...
						{
							$this->Flash->error(__('La transacción no pudo ser insertada'));
		...
		$this->Flash->success(__('Estudiantes de 5to. Año egresados ' . $contadorEstudiantes5to));
		...
		$nivelBuscado = 'Secundaria, 1er. año';
			...
			foreach ($estudiantesBecados as $becado)
			{
				...
				if ($contadorTransaccionesBecado > 0)
				{
					foreach ($transaccionesBecado as $transaccion)
					{
						...
						{
							$contadorActualizadas++;
							$this->Flash->success(__('Transacción con ID ' . $transaccion->id . ' fue actualizada'));
						} 
						else 
						{
							$contadorNoActualizadas;
    ...
    public function addAdminb($idParentsandguardians = null)
    {
        ...
        if ($this->request->is('post')) 
        {
			...
			if ($row)
			{
				$this->Flash->error(__('Ya existe otro estudiante con el mismo número de documento de identidad ' . $student->type_of_identification . $student->identity_card));
			}
			else
			{
				$student->user_id = 3;
