...
class BinnaclesController extends AppController
{
                ...
                foreach ($columnasMensualidades as $indice => $valor)
                {
                    if ($estudiante->$indice > 0)
                    {
                        ...
                        elseif ($mensualidadAplicar < $estudiante->$indice)
                        {
                            ...
                            $montoAbonado = $mensualidadAplicar;
    ...
    public function cargarMensualidadesSeptiembreBecados()
    {
        $nivelNuevo = 
        [
            'Pre-escolar, pre-kinder' => 'Pre-escolar',                                
        ...
        foreach ($estudiantes as $estudiante)
        {
            ...
            if ($ultimaFila)
            {
                ...
                $idRepresentante = $ultimaFila->parentsandguardian_id;
                $nombreEstudiante = $ultimaFila->surname . ' ' . $ultimaFila->first_name; 
                $descuentoEstudiante = $ultimaFila->discount;
                $descuentoEstudianteAplicar = 0;
                if ($nivelEstudianteNuevo == 'Primaria')
                {
                    $matriculaAplicar = $mensualidadPrimaria;
                    if ($ultimaFila->discount > 0)
                    {
                        $mensualidadAplicar = round($mensualidadPrimaria - (($mensualidadPrimaria * $ultimaFila->discount)/100), 2);
                    }
            ...
            if ($estudianteEncontrado == 1)
            {
                if ($estudiante->columna_extra27 > 0)
                {
                    $pagoMensualidad = 1;
                    $contadorTransaccionesMayorCero++;
                    $montoAbonado = $estudiante->columna_extra27;
                    if ($mensualidadAplicar == $estudiante->columna_extra27)
                    {
                        $pagoTotal = 1;
                        $pagoParcial = 0;
                        $descuentoEstudianteAplicar = $descuentoEstudiante;
                    }
                    elseif ($mensualidadAplicar > $estudiante->columna_extra27)
                    {
                        $pagoTotal = 0;
                        $pagoParcial = 1;
                    }
                    elseif ($mensualidadAplicar < $estudiante->columna_extra27)
                    {
                        ...
                        [
                            ...
                            'nivel' => $nivelEstudianteNuevo,
                            'monto' => $mensualidadAplicar, 
                            'abono' => $estudiante->columna_extra27,
                            'id_representante' => $idRepresentante
                        ];

                        $montoAbonado = $mensualidadAplicar;
                        $sobranteMensualidad = $estudiante->columna_extra27 - $mensualidadAplicar;
                    }
                    ...
                    [
                        ...
                        'ano_escolar' => '2021',
                        'tipo_transaccion' => 'Mensualidad',
                        'descripcion_transaccion' => 'Sep 2021',
                        'monto' => $mensualidadAplicar,
                        'monto_original' => $mensualidadAplicar,
                        'pago_total' => $pagoTotal,                          
                        'pago_parcial' => $pagoParcial,
                        'monto_abonado' => $montoAbonado,
                        'porcentaje_descuento' => $descuentoEstudianteAplicar
                    ];

                    $this->Flash->success(__('id Estudiante ' . $idEstudiante . ' Cédula: ' . $cedula . ', Nivel estudio: ' . $nivelEstudianteNuevo . ', Mensualidad a aplicar: ' . number_format($mensualidadAplicar, 2, ",", ".") . ', Monto abonado: ' . number_format($montoAbonado, 2, ",", "."))); 
                    
                    $codigoRetornoTransaccion = $this->actualizarTransaccion($datosTransaccion);
                    if ($codigoRetornoTransaccion == 1)
                    {
                        $contadorTransaccionesNoGuardadas++;
                        $transaccionesNoGuardadas[] = 
                        [
                            'cedula' => $cedula,
                            'mensualidad' => 'Sep 2021',
                            'monto' => $estudiante->columna_extra27
                        ];
                    }
                    
                }
                
                if ($pagoMensualidad == 1)
                {
                    $registroEstudiante->balance = 2021;
                    if (!($this->Students->save($registroEstudiante)))
                    {
                ...
                if ($sobranteMensualidad > 0)
                {
                    ...
                    if ($ultimaFila)
                    {
                        $ultimaFila->balance = round($ultimaFila->balance + $sobranteMensualidad, 2);
    ...
    public function actualizarTransaccion($datosTransaccion = null)
    {
        $codigoRetorno = 0;
        ...
        ->where(['student_id' => $datosTransaccion['id_estudiante'], 'transaction_description' => 'Sep 2021'])
        ->order(['created' => 'DESC']);
        
        $transaccionEstudiante = $transacciones->first();

        if ($transaccionEstudiante)
        {       
            $transaccionEstudiante->payment_date = $datosTransaccion['fecha_pago'];
            $transaccionEstudiante->ano_escolar = $datosTransaccion['ano_escolar'];
            $transaccionEstudiante->transaction_type = $datosTransaccion['tipo_transaccion'];
            ...
            $transaccionEstudiante->transaction_migration = 0;
            $transaccionEstudiante->amount_dollar = $datosTransaccion['monto_abonado'];
            $transaccionEstudiante->porcentaje_descuento = $datosTransaccion['porcentaje_descuento'];
